...
"""
This is a program for detecting an increase in the variance by a SR procedure with
constant known mean, known initial s.d. and unknown post-change variance,
with a prior on 1/eta2 that is a Gamma(alpha, beta) distribution conditional on being <1
...
# def increase_in_var_known_mean(x: pd.DataFrame, mu: float, sigma: float, alpha: float, beta: float, cutoff: float):
def prog03(x: pd.DataFrame, mu: float, sigma: float, alpha: float, beta: float, cutoff: float):
    z = ( x - mu ) / sigma
    _, length = z.shape
